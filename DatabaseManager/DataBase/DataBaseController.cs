        ...
        public MeterInformation GetMeterSettings(string MSN, bool chechForOnDemand = false, long parent_id = -1)
        {
            ...
            try
            {
                ...
                query = "SELECT `feeder_id`,`msn`,`parent_id`, `global_device_id` ,`password`,`type`,`network_ip`,`device_type`,`llc_protocol_type`,`model_id`, `status`, `unset_ma`,`hdlc_Address`,`schedule_type`, "
                    ...
                    + "`new_password_activation_time`, `max_cs_difference`,`apply_new_contactor_state`,`new_contactor_state`,`mdi_reset_date`,`mdi_reset_time`,`write_mdi_reset_date`,`default_password`,`last_password_update_time`,"
                ...
                if (DT.Rows.Count > 0)
                {
                    ...
                    if(DT.Rows[0][DT.Columns["mdi_reset_time"]] == DBNull.Value || !TimeSpan.TryParse(DT.Rows[0][DT.Columns["mdi_reset_time"]].ToString(),out tempMeterInfo.Mdi_reset_time))
                    {
                        throw new Exception("Invalid Mdi reset time.");
                    }
