    ...
    public class DataBaseController
    {
        ...
        public DataBaseController()
        {
            try
            {
                ...
                connection.ConnectionString = $" Data Source = {DBConnect.ConnectionString}";
        ...
        public User verifyUserAndGetDetails(string userName, string password)
        {
            ...
            try
            {
                ...
                {
                    hashPassword = Common.Commons.ArrayToHexString(mySHA256.ComputeHash(ASCIIEncoding.ASCII.GetBytes(password)), true);
                }
        ...
        */
        ...
        public bool addNewUser(User obj)
        {
            ...
            try
            {
                ...
                cmd = "INSERT INTO [user] (isActive,user_type_id, user_rights_id,rights_id_r326,rights_id_r411,rights_id_t421,rights_id_r421,rights_id_r283, user_name, user_password, father_Name, address, phone_1, phone_2, mobile_No, fax_No, nID, employee_Code, creationDate) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,{SqliteCurrentDateTime})";
                command = new SQLiteCommand(cmd, connection);
                ...
                using (SHA256 mySHA256 = SHA256Managed.Create())
                {
                    obj.userPassword = Common.Commons.ArrayToHexString(mySHA256.ComputeHash(ASCIIEncoding.ASCII.GetBytes(obj.userPassword)), true);
...
*/
        ...
        public bool CreateNewAccessRights(string idendifier, string role, ApplicationRight rights)
        {
            try
            {
                ...
                cmd.Parameters.Add(new SQLiteParameter("@identifier", idendifier) { DbType = DbType.String });
                cmd.Parameters.Add(new SQLiteParameter("@role", role) { DbType = DbType.String });
                cmd.Parameters.Add(new SQLiteParameter("@rights", ApplicationRight.GetBinaryObject(rights)) { DbType = DbType.Binary });
                OpenConnection();
        ...
        public bool UpdateAccessRights(int id, string idendifier, string role, ApplicationRight rights)
        {
            try
            {
                ...
                cmd.Parameters.Add(new SQLiteParameter("@rights", ApplicationRight.GetBinaryObject(rights)) { DbType = DbType.Binary });
                //cmd.Parameters.Add(new SQLiteParameter("@id", id) { DbType = DbType.BigInt });
                OpenConnection();
                cmd.Connection = connection;
                cmd.ExecuteNonQuery();
                return true;
        ...
        public bool DeleteAccessRights(int id)
        {
            try
            {
                ...
                cmd.Parameters.Add(new SQLiteParameter("@id", id) { DbType = DbType.UInt64 });
                OpenConnection();
                cmd.Connection = connection;
                cmd.ExecuteNonQuery();
                return true;
        ...
        public ApplicationRight SelectAccessRights(int id, int meterInfoId, ref SQLiteConnection con)
        {
            ApplicationRight RightObj = null;
            try
            {
        ...
        public bool saveCumulativeBillingData(Cumulative_billing_data Data)
        {
            try
            {
                if (OpenConnection())
                {
                    ...
                    string query = $"INSERT INTO [cumm_billing_data] VALUES(?,{SqliteCurrentDateTime}, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        public bool saveCumulativeBillingData_SinglePhase(cumulativeBilling_SinglePhase Data)
        {
            ...
            {
                connection.Open();
                string query = $"INSERT INTO [cumm_billing_data_singlephase] VALUES(?,{SqliteCurrentDateTime}, ?, ?, ?)";
        ...
        public bool saveMonthlyBillingData_SinglePhase(Monthly_Billing_data_SinglePhase MB_data)
        {
            try
            {
                string query = $"INSERT INTO [monthly_billing_data_singlephase] VALUES(?,{SqliteCurrentDateTime}, ?, ?, ?, ?)";
                connection.Open();
        ...
        public bool saveAlarmStatus(string MSN, BitArray AlarmStatus)
        {
            try
            {
                ...
                MyInsertQuery += ")";
                command = new SQLiteCommand(MyInsertQuery, connection);
                int c = command.ExecuteNonQuery();
                if (c > 0)
                {
                    return true;
        ...
        public bool SavePowerQuality_SinglePhase(Instantaneous_SinglePhase Data)
        {
            ...
            {
                connection.Open();
                string query = $"INSERT INTO [instantaneous_data_singlephase] VALUES(?,{SqliteCurrentDateTime}, ?, ?, ?, ?, ?, ?)";
        ...
        public bool SavePowerQuality(InstantaneousObjClass Data)
        {
            try
            {
                connection.Open();
                SQLiteParameter T = null;
                string query = $"INSERT INTO [instantaneous_data] VALUES(?,{SqliteCurrentDateTime},?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
