...
{
    public delegate void Connected(IList clientSock);
    public class MTI_TCP_Server : IDisposable
    {
        ...
        private int maxConlistenCount = 02;
        ...
        public void ListenPort()
        {
            try
            {
                ...
                {   
                    ...
                    Thread NotificationThread = new Thread(Invoke_ConnectionReceived);
                    ...
                    //        _serverSocket.BeginAccept(new AsyncCallback(AcceptClient), _serverSocket);
                    //    else
                    //        throw new Exception("Server Socket disconnected");
                    //    allDone.WaitOne(Timeout.Infinite, false);
                    //}
                    while (true)
                    {
                        ...
                        //        if (task == null || task.Status == TaskStatus.Faulted)// || task.Status == TaskStatus.RanToCompletion || task.Status == TaskStatus.Canceled)
                        //        {
                        //            task = Task.Factory.StartNew((x) => ConnectionReceived.Invoke((IList)x), clientSocks);
                        //        }
                        //        Tasks[index] = task;
                        //        Console.Out.WriteLine(String.Format("Task status {0} Count {1}", task.Status, clientSocks.Count));
                        //    }
                        //}
                    }
                }
            ...
            catch (Exception exc)
            {
                ...
                Thread.CurrentThread.Abort();
                throw new Exception("Unable to receive client connection");
            }
            finally
            {
                Dispose();
        ...
        public void Invoke_ConnectionReceived()
        {
            try
            {
                while (clientSocks != null)
                {
                    if (clientSocks.Count > 0)
                    {
                        ConnectionReceived.Invoke(clientSocks);
                    }
                    else
                        Thread.Sleep(200);
                }
        ...
        public void AcceptClient()
        {
            ...
            {
                MessageBox.Show(exc.Message, "Port already closed");
            }
