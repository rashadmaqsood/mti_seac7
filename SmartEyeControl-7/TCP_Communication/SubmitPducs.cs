...
namespace ReadSMS_AT_CS20
{
    public class SubmitPdu
    {
        ...
        public static string GetPdu(string number, byte[] message, string smscAddress,
            out int actualLength)
        {
            ...
            builder.Append(IntToHex(0x04)); // Data Coding Scheme, 04=8-Bit
        ...
        private static byte[] EncodeText(byte[] data, out byte dataLength)
        {
            ...
            return SeptetsToOctetsInt(data); // DEFAULT
        }
        ...
        /// </remarks>
        private static byte[] SeptetsToOctetsInt(byte[] data)
        {
            int size_of_data = data.Length;
            byte[] return_data = new byte[size_of_data];
            return_data = data;
            ...
            //{
            //    array[i] = (byte)(Convert.ToChar( data.Substring(i,1)));

            //    string current = IntToBin((byte)data[i], 8);
            //    if (i != 0 && i % 8 != 0)
            //    {
