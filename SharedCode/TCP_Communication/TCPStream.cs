    ...
    public class TCPStream : NetworkStream, IComparable<TCPStream>
    {
            ...
            : base(socket, access, ownsSocket)
        {
            try
            {
                ...
                base.Socket.SetSocketOption(SocketOptionLevel.Socket, SocketOptionName.SendBuffer, MaxMeterBufferLength);
                base.Socket.SetSocketOption(SocketOptionLevel.Socket, SocketOptionName.ReceiveBuffer, MaxMeterBufferLength);
                // base.Socket.SetSocketOption(SocketOptionLevel.IP, SocketOptionName.DontFragment, false);
                base.Socket.DontFragment = false;
        ...
        public bool TryProcessHeartBeat()
        {
            ...
            try
            {
                ...
                {
                    int length = base.Read(LocalBuffer.Array, 0, LocalBuffer.Array.Length);
        ...
        public override void Write(byte[] buffer, int offset, int size)
        {
            try
            {
                ...
                base.Write(Buffer_Arg.Array, Buffer_Arg.Offset, Buffer_Arg.Count);
                base.Flush();
        ...
        public override int Read(byte[] buffer, int offset, int size)
        {
            try
            {
                ...
                while (true)
                {
                    localBufferArg = new ArraySegment<byte>(buffer, offset, size);
                    ArraySegment<byte> _localBuffer = LocalBuffer;
                    int length = base.Read(_localBuffer.Array, _localBuffer.Offset, _localBuffer.Count);
                ...
                while (arrBuf != null && offset < arrBuf.Length && (offset - Offset) < length)
                {
                    ...
                    try
                    {
                            ...
                            HeartBeat.HeartBeatMaxLength <= dtLength)
                        {
                            ...
                            if (!OverlayMode && !hbeat.IsVerifited)
                            {
                            ...
                            if (hbeat.IsVerifited)
                            {
                                offset += HeartBeat.HeartBeatMaxLength;
                            }
                            else if (tcpWrapper.IsVerified)
                            {
        ...
        public bool TryProcessReceiveDataASync()
        {
            ...
            {
                // Check Read is Non-block
                if ((CommunicationMode == CommunicationMode.IdleAliveMode || OverlayMode) &&
                    DataAvailable)
                {
                    ArraySegment<byte> _LocalBuffer = LocalBuffer;
        ...
        public bool TryWriteHeartBeatResponse()
        {
            try
            {
                base.Write(HeartBeat.HeartBeatResponse,0,HeartBeat.HeartBeatResponse.Length);
                base.Flush();
                return true;
            }
            catch (Exception) { }
