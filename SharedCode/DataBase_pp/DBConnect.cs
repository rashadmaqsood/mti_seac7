    ...
    public class DBConnect
    {
        ...
        public static string ConnectionString 
        {
            get 
            {
                ...
                try
                {
                    ConnString = Settings.Default.SEAC7_DBConnectionString;
                    //ConnString = @"Provider=Microsoft.ACE.OLEDB.12.0;Data Source=Application_Configs\SEAC7_DB.mdb;Persist Security Info=True;Jet OLEDB:Database Password=$v#1S0ot@5";
                }
                catch
                {
                    //ConnString = @"Provider=Microsoft.ACE.OLEDB.12.0;Data Source=Application_Configs\SEAC7_DB.mdb;Persist Security Info=True;Jet OLEDB:Database Password=$v#1S0ot@5";
        ...
        public string SaveToDataBase(List<Insert_Record> List_Records, string category, uint max_Counter)
        {
            try
            {
                establishConnection();
        ...
        {
            OleDbDataReader reader_maxCounter = null;
            int max_Counter = -1;
            try
            {
                establishConnection();
                if (this.OpenConnection() == -1)
                {
                    ...
                    OleDbCommand cmd = new OleDbCommand(queryForMaxCounter, connection);
                    if (ServerConnected == true)
                    {
        ...
        //Terrification+++++
        public bool save_TerrificationToDatabase(comm.Param_ActivityCalendar Arg, string MSN)
        {
        ...
        public void callAllTerrificationFunctions(comm.Param_ActivityCalendar Arg, string MSN)
        {
            //calendar info
            saveCalendar(Calendar_Obj.CalendarName, comm.SeasonProfile.MAX_Season_Profiles, comm.TimeSlot.MAX_TimeSlot, MSN);
            //save day Profile
        ...
        public void saveSeasonProfile(comm.Param_SeasonProfile season, string MSN)
        {
            try
            {
                ServerConnected = true;
        ...
        {
            OleDbDataReader reader_lastCalendarID = null;
            string temp_C_id = "";
            try
            {
                string queryForCid = "SELECT MAX([cal_id]) FROM calendar_groups WHERE [cal_name] LIKE '%" + meterSerialNumber + "'";
                OleDbCommand cmd = new OleDbCommand(queryForCid, connection);
                if (ServerConnected == true)
                {
        ...
        public void saveWeekProfile(comm.WeekProfile item, string MSN)
        {
            try
            {
        ...
        {
            OleDbDataReader reader_lastSeasonID = null;
            string temp_sp_id = "";
            try
            {
                string queryForSPid = "SELECT MAX(sp_id) FROM season_profile WHERE sp_name LIKE '%" + meterSerialNumber + "'";
                OleDbCommand cmd = new OleDbCommand(queryForSPid, connection);
                if (ServerConnected == true)
                {
        ...
        public void saveDayProfile(comm.Param_DayProfile dayProfile)
        {
            ...
            foreach (var dayItem in dayProfile)
            {
                ...
                //create command and assign the query and connection from the constructor
                OleDbCommand cmd = new OleDbCommand(query_DayProfile, connection);
                ...
                for (int i = 0; i < TimeSliceCount; i++)
                {
                    ...
                    //create command and assign the query and connection from the constructor
                    OleDbCommand cmd1 = new OleDbCommand(query_Tariff, connection);
        ...
        {
            OleDbDataReader reader_last_DP_ID = null;
            string temp_DP_id = "";
            try
            {
                string queryForDPid = "SELECT MAX(dp_id) FROM day_profile WHERE dp_name LIKE '%" + meterSerialNumber + "'";
                OleDbCommand cmd = new OleDbCommand(queryForDPid, connection);
                if (ServerConnected == true)
                {
        ...
        public void saveSpecialDays(comm.Param_SpecialDay SD, string MSN)
        {
            ...
            foreach (var item in SD)
            {
                ...
                //create command and assign the query and connection from the constructor
                OleDbCommand cmd = new OleDbCommand(query_SP, connection);
        ...
        public void update_Meter_CalendarID(string MSN)
        {
            ...
            try
            {
                ...
                OleDbCommand cmd = new OleDbCommand(queryForMeter_CalID_update, connection);
                if (ServerConnected == true)
                {
                    //executing query
                    cmd.ExecuteNonQuery();
        ...
        public bool save_session_info(string MSN, string category)
        {
            ...
            try
            {
                ...
                //assigning command
                OleDbCommand cmd = new OleDbCommand(queryForSession, connection);
                if (ServerConnected == true)
                {
                    //executing query
                    cmd.ExecuteNonQuery();
        ...
        {
            OleDbDataReader reader_loadProfile = null;
            try
            {
                string queryForSessionID = "SELECT MAX([id]) as pp FROM [session] WHERE msn = '" + MSN + "'";
                OleDbCommand cmd = new OleDbCommand(queryForSessionID, connection);
                if (ServerConnected == true)
                {
        ...
        public void update_session_info()
        {
            try
            {
                ...
                OleDbCommand cmd = new OleDbCommand(queryForSession_info_update, connection);
                if (ServerConnected == true)
                {
                    //executing query
                    cmd.ExecuteNonQuery();
        ...
        public void save_Info_To_meterList(string MSN)
        {
            try
            {
                ...
                OleDbCommand cmd = new OleDbCommand(queryForMeterList, connection);
                if (ServerConnected == true)
                {
                    //executing query
                    cmd.ExecuteNonQuery();
        ...
        public void update_maxCounter(int MAX, string catagory, string MSN)
        {
            try
            {
                ...
                OleDbCommand cmd = new OleDbCommand(queryForUpdateMaxCounter, connection);
                if (ServerConnected == true)
                {
                    //Execute command
                    cmd.ExecuteNonQuery();
