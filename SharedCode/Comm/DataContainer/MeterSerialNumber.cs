...
{
    public class MeterSerialNumber : IComparable<MeterSerialNumber>, IEqualityComparer<MeterSerialNumber>
    {
        #region Data_Members
        ...
        static MeterSerialNumber()
        {
            ...
            HLManufacturerId = 99;
            HLMeterType = 99;
            HLMeterSerialNo = 999999;
            ///Populate Meter Models Str
            Manufacturer_ModelTokens = new Dictionary<string, uint>();
            ///Manually Load Strings Here BCD format
            ///later may load from system configuration file
            Manufacturer_ModelTokens.Add("MTI_R326_", 0x2462);
            //Manufacturer_ModelTokens.Add("MTI_R326_", 0x2062);
            Manufacturer_ModelTokens.Add("R326_", 0x2462);
            Manufacturer_ModelTokens.Add("RGCM_", 0x240B);
            ...
            Manufacturer_ModelTokens.Add("ACC_ACT34G_", 0x1D62);
            Manufacturer_ModelTokens.Add("ACT34G_", 0x1D62);
        }
        ...
        public string GetMeterModel()
        {
            ...
            {
                model = GetMSNPrefix();
                string[] splits = model.Split(new char[] { '_' });
                model = splits[splits.Length - 1];
                if (string.IsNullOrEmpty(model)) model = "R326";
            }
            catch (Exception)
            {
            }
        ...
        public static MeterSerialNumber ConvertFrom(byte[] RawMSN, int index, int length)
        {
            try
            {
                ...
                //if (RawMSN != null && RawMSN.Length >= 4)
                //{
                //    number = number << 8;
                //    number = number | RawMSN[index + 1];
                //    number = number << 8;
                //    number = number | RawMSN[index + 2];
                //    number = number << 8;
                //    number = number | RawMSN[index + 3];
                //}
        ...
        #region IEqualityComparer<MeterSerialNumber>

        public bool Equals(MeterSerialNumber other)
        {
            if (ReferenceEquals(null, other)) return false;
            if (ReferenceEquals(this, other)) return true;
            return MSN == other.MSN;
        }
        ...
        public override bool Equals(object obj)
        {
            ...
            if (obj.GetType() != typeof(MeterSerialNumber)) return false;
            return MSN == ((MeterSerialNumber)obj).MSN;
        }

        bool IEqualityComparer<MeterSerialNumber>.Equals(MeterSerialNumber x, MeterSerialNumber y)
        {
            if (ReferenceEquals(null, x)) return false;
            if (ReferenceEquals(x, y)) return true;
            if (x.GetType() != typeof(MeterSerialNumber)) return false;
            return x.MSN == y.MSN;
        }
        ...
        int IEqualityComparer<MeterSerialNumber>.GetHashCode(MeterSerialNumber obj)
        {
