...
namespace SharedCode.Comm.Param
{
    ...
    public class NetworkModes
    {
        ...
        public byte Priority3 { get; set; }
        public byte Priority4 { get; set; }
        public byte NetworkModeDetail
        {
            get
            {
                byte val = (byte)(AutoSelection ? 0 : 1);
                val = (byte)((val << 6) | (Priority1 << 4));
                val = (byte)(val | (Priority2 << 2));
                val = (byte)(val | (Priority3));
                return val;
            }
            set
            {
                AutoSelection = !Convert.ToBoolean(value >> 6);
                Priority1 = (byte)((0x30 & value) >> 4);
                Priority2 = (byte)((0x0C & value) >> 2);
                Priority3 = (byte)(0x03 & value);
            }
        }
    }
    [Serializable]
    ...
    public class Param_Communication_Profile:ICustomStructure,IParam
    {
        ...
        [XmlIgnore()]
        public NetworkModes Network_Mode = new NetworkModes();

        [XmlElement("NETWORK_MODE_DETAIL", typeof(byte))]
        public byte NETWORK_MODE_DETAIL
        {
            get
            {
                return Network_Mode.NetworkModeDetail;
            }
            set
            {
                Network_Mode.NetworkModeDetail = value;
            }
        }
        ...
        public byte[] Encode_Data()
        {
            try
            {
                ...
                encodeRaw.AddRange(BasicEncodeDecode.Intelligent_Data_Encoder(DataTypes._A11_unsigned, Network_Mode.NetworkModeDetail));
        ...
        public void Decode_Data(byte[] Data, ref int array_traverse,int length)
        {
            try
            {
                ...
                Network_Mode.NetworkModeDetail = Convert.ToByte(BasicEncodeDecode.Intelligent_Data_Decoder(ref Data, ref array_traverse));
